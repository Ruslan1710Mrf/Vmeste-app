...
export function mergeProfiles(firestoreProfile, localProfile, user) {
  ...
  return {
    ...
    bio: firestoreProfile.bio || base.bio,
    interests: firestoreProfile.interests?.length ? firestoreProfile.interests : base.interests,
    memberSince: firestoreProfile.memberSince || base.memberSince,
  };
}
...
export function profileToFirestoreDoc(profile) {
  return {
    name: profile.name,
    email: profile.email ?? '',
    city: profile.city,
